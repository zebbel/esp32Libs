...
void CRSF::init(uart_port_t uartNumVal, QueueHandle_t *queue){
    extern_queue = queue;
    init(uartNumVal);
}
    ...
    for (;;) {
        ...
        if (xQueueReceive(crsf->uart_queue, (void *)&event, (TickType_t)portMAX_DELAY)) {
            ...
            if (event.type == UART_DATA ) {
                ...
                // Broadcast Frame
                if(frame.type < 0x27){
                    memcpy(&frame.payload, &dtmp[3], frame.len - 2);
                ...
                }else if(crsf->extern_queue != NULL){
                    frame.destAddr = dtmp[3];
                    // only handle frame if device address matches
                    if(frame.destAddr == CONFIG_CRSF_DEVICE_ADDRESS){
                        frame.srcAddr = dtmp[4];
                        memcpy(&frame.payload, &dtmp[5], frame.len - 4);

                        //crsf->handleExtendedFrame(frame);
                        switch(frame.type){
                            case CRSF_FRAMETYPE_PARAMETER_WRITE:
                                xQueueSend(*crsf->extern_queue, &frame, portMAX_DELAY);
                                break;
                        }
                    }
                }
